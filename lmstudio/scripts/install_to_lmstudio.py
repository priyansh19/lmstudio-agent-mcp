...
MAIN_CONFIG = LMSTUDIO / "mcp" / "mcp.json"
KEYS_CONFIG = LMSTUDIO / "mcp" / "optional-with-keys.json"
...
def _resolve_paths(servers: dict, lmstudio: str, root: str, sandbox: str) -> None:
    """Replace portable placeholders so the config works on any machine."""
    home = str(Path.home())

    def sub(s: str) -> str:
        return (s.replace("__LMSTUDIO__", lmstudio)
                 .replace("__OPENCLAW__", root)
                 .replace("__ROOT__", root)
                 .replace("__REPO__", lmstudio)
...
def main() -> None:
    ...
    _resolve_paths(incoming, str(LMSTUDIO), str(ROOT), sandbox)
