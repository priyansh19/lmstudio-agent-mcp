...
#   prerequisites → Python deps → MCP servers → GitHub → auto-start bridge
#
# Usage:
...
cd "$(dirname "$0")"
LMSTUDIO="$(pwd)"
ROOT="$(dirname "$LMSTUDIO")"
REPO="$LMSTUDIO"
export PATH="/opt/homebrew/bin:/usr/local/bin:${HOME}/.local/bin:${PATH}"

# Default git identity from config/user-identity.json (override with env vars)
_identity_file="$REPO/config/user-identity.json"
if [ -f "$_identity_file" ] && command -v python3 >/dev/null 2>&1; then
  _DEFAULT_GIT_NAME="$(python3 -c "import json; print(json.load(open('$_identity_file')).get('git_name',''))" 2>/dev/null || true)"
  _DEFAULT_GIT_EMAIL="$(python3 -c "import json; print(json.load(open('$_identity_file')).get('git_email',''))" 2>/dev/null || true)"
  export _DEFAULT_GIT_NAME _DEFAULT_GIT_EMAIL
fi
...
step_github() {
  ...
  local name="${GIT_NAME:-${_DEFAULT_GIT_NAME:-$(ask "Git author name" "$(git config --global user.name 2>/dev/null || echo "")")}}"
  local email="${GIT_EMAIL:-${_DEFAULT_GIT_EMAIL:-$(ask "Git author email" "$(git config --global user.email 2>/dev/null || echo "")")}}"
...
# --------------------------------------------------------------------------- #
# STEP 8 — Auto-start bridge on login
# --------------------------------------------------------------------------- #
step_launchagent() {
  step 8 "Auto-start bridge on Mac login (LaunchAgent)"
...
print_finish() {
  ...
  info "  • LM Studio MCP tools     → ~/.lmstudio/mcp.json (incl. think-delegate, triage, memory-rag)"
  info "  • Claude escalation       → deep_think via Claude CLI subscription"
  info "  • OpenAI bridge           → http://127.0.0.1:8765/v1 (local/current)"
  info "  • Bridge auto-start       → ~/Library/LaunchAgents/com.lmstudio-agent.bridge.plist"
  info "  • Sandbox                 → $SANDBOX"
  echo
  bold "Every time you use it:"
  info "  1. LM Studio:  lms server start  &&  lms load   (LLM + embedding model for RAG)"
  info "  2. LM Studio:  paste prompts/system-prompt.md into Chat → System Prompt"
  info "  3. LM Studio:  toggle MCP servers on (Program → mcp.json)"
  info "  4. Terminal:   uv run python agent/local_agent.py --root \"$SANDBOX\""
  echo
