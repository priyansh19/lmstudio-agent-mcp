...
def main() -> None:
    ...
    from agent.rag_context import augment_user_message as rag_augment, save_turn
    ...
    def run_task(task: str) -> None:
        ...
        if not args.no_triage:
            ...
            if triage.route == "claude" and triage.expert_response:
                ...
                if not args.no_rag:
                    save_turn(task, triage.expert_response, workspace=root)
                print()
                return
        ...
        if assistant_text and not args.no_rag:
            save_turn(task, assistant_text, workspace=root)
        print()
