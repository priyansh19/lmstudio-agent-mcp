...
def memory_rag_status() -> str:
    """Show Phase 2 vector memory configuration and store counts."""
    cfg = load_config()
    store = VectorMemoryStore()
    return "\n".join([
        "memory-rag MCP — Phase 2 semantic + episodic vector RAG",
        ...
        "",
        "Tools: remember_fact, remember_episode, search_semantic, search_episodic, rag_preview",
    ])
