...
After this runs, OpenClaw always uses:
    provider: local-agent
    model:    local-agent/local/current

Swap models in LM Studio only — never touch OpenClaw config again.

Usage:
    uv run python scripts/setup_openclaw_lmstudio.py
    uv run python scripts/setup_openclaw_lmstudio.py --with-mcp   # also sync MCP tools
    uv run python scripts/setup_openclaw_lmstudio.py --dry-run
"""
...
def main() -> None:
    ...
    print(f"Primary model: local-agent/local/current")
    print(f"Bridge URL:    {BRIDGE_URL}")
    print("\nNext:")
    print("  1) Start bridge:  uv run python agent/lmstudio_bridge.py")
